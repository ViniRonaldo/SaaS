...
export default function UsuarioAgendamentosPage() {
  ...
  useEffect(() => {
    patientService.getMe()
      .then((patient) => patient?.id ? appointmentService.getAll({ patientId: patient.id, status: 'SCHEDULED' }) : null)
            ...
            <tbody className="divide-y divide-gray-100">
              {appointments.map((a) => (
                <tr key={a.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-700">{a.doctor?.name || '-'}</td>
                  <td className="px-6 py-4 text-sm text-gray-700">{a.healthUnit?.name || '-'}</td>
                  <td className="px-6 py-4 text-sm text-gray-700"><span className="inline-flex items-center gap-1"><Calendar size={13}/>{a.date?.split('T')[0]} {a.startTime}</span></td>
